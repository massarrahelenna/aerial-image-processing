document.addEventListener("DOMContentLoaded", function () {
      ...
      const homeTeamDisplay = document.getElementById("homeTeamDisplay");
      const awayTeamDisplay = document.getElementById("awayTeamDisplay");
      const videoElement = document.getElementById("tactical-video");
      ...
      form.addEventListener("submit", (e) => {
        ...
        if (!homeTeam || !awayTeam || !videoFile) {
          alert("Por favor, preencha todos os campos e selecione um vídeo.");
          return;
        }

        // Atualiza nomes dos times na interface
        homeTeamDisplay.textContent = homeTeam;
        awayTeamDisplay.textContent = awayTeam;

        // Atualiza vídeo para o arquivo enviado pelo usuário
        const videoURL = URL.createObjectURL(videoFile);
        videoSource.src = videoURL;

        // Recarrega o vídeo no player Video.js
        videoElement.load();
        ...
        player.addEventListener("click", function () {
          if (this.textContent) {
            ...
            document.getElementById("playerPosition").textContent =
              this.parentElement === document.querySelector(".formation-grid") ? "Atacante" : "Meio-campo";
            document.getElementById("playerDistance").textContent = `${Math.round(Math.random() * 10)}.${Math.round(
              Math.random() * 9
            )} km`;
            document.getElementById("playerPasses").textContent = `${Math.round(Math.random() * 30)}/${Math.round(
              Math.random() * 40
